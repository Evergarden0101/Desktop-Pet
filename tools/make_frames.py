...
For every ``assets/pets/<name>/base.png`` this builds:

  * ``crawl_000.png`` ... an 8 frame crawl cycle (hands and knees, alternating
    limbs, body bob and rock)
  * ``play_000.png`` ...  a 6 frame idle/play cycle (sitting up, waving)
  * ``meta.json``         frame list, fps and the head anchor per frame

How the pose is built: the cutout is sliced into head / torso / thigh / shin
layers along the alpha silhouette, and each layer is rotated about its joint.
The figure's own pixels are never repainted, so the face, hairstyle and
clothing survive exactly as they were.  Only the arms and hands -- which a
standing photo does not show in a crawling pose -- are drawn, in the skin and
clothing colours sampled from that same figure.
...
from rig import (analyze, slice_parts, part_pivot, paste_rotated, draw_arm,  # noqa: E402
...
def main() -> int:
    ...
    for folder in folders:
        try:
            build_figure(folder)
        except Exception as exc:  # keep going, one bad cutout should not stop the batch
            print(f"  {folder.name}: FAILED ({exc})", file=sys.stderr)

    print("\nnext:  python -m desktop_pet   (from src/)  or  python run.py")
    return 0
